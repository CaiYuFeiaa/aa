<template>
    <div style="height:100%;">
        <Menu theme="dark" active-name="8" style="height:100%;vertical-align:top;display:inline-block;">
            <MenuGroup title="用户">
        ...
        <div style="width:1270px;height:100%;vertical-align:top;border:1px solid black;display:inline-block;position:relative;">
            <div style="width:100%;">
                <ul style="list-style:none;">
                    <li class="litb">
                        <span style="width:150px;">类型id</span>
                        <span style="width:80px;">类型缩略图</span>
                        <span style="width:150px;">类型代号</span>
                        <span style="width:200px;">类型名称</span>
                    </li>

                    <li class="litb" v-for="item in showList.slice(0, 6)">
                        <span style="width:150px;">{{item.categoryId}}</span>
                        <img :src="item.categoryImg" style="width:50px;margin:0 15px;vertical-align:middle;"/>
                        <span style="width:150px;">{{item.categoryType}}</span>
                        <span style="width:200px;">{{item.categoryName}}</span>
                    </li>
                </ul>
            </div>
    ...
    export default {
        ...
        created: function () {
            ...
            axios.get('http://localhost:8090/fy/category/index')
                .then((res) => {
                    this.categoryImgList = res.data.data
        ...
        computed: {
            total() {
                return this.categoryImgList.length;
            },
        },
        methods:{
            change(page){
                ...
                for (var i = 0; i < this.size; i++) {
                    ...
                    if (index < this.categoryImgList.length) {
                        var prod = this.categoryImgList[index];
